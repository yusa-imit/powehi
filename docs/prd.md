...
- **`(group_id, device_id, created_at_epoch, created_at)` — 대기 중인 MLS Remove 알림** (§5.4 revocation 알림 채널, cycle 448): 디바이스가 revoke되면 서버는 그 디바이스가 속해 있던 각 그룹에 대해 "남은 멤버들이 아직 이 device_id에 대한 MLS Remove를 issue해야 한다"는 알림 행을 `pending_removals` 테이블에 영구 저장함. 값 자체(어떤 device가 어떤 그룹에 속해 있었는지)는 위 `group_members` 항목으로 서버가 이미 알던 사실과 같은 카테고리이지만, `media_acks`/`envelope_acks`와 마찬가지로 **이미 알던 사실을 새로 영구 저장**한다는 점이 델타임: 변경 전에는 `devices` 행 삭제 시 `group_members` cascade로 이 소속 기록이 DB에서 완전히 사라졌으나, 이제는 revoke 이후에도 "device D가 시각 T(`created_at`)에 그룹 G에 속한 채로 revoke되었다"는 사실이 (해당 그룹의 남은 멤버가 실제로 `remove_member`를 호출하고 그 시점 이후 그룹에 어떤 Commit이든 하나 착지할 때까지) 무기한 잔존함 — 현재 명시적인 보존 상한(TTL/GC)이 없음. 완화: `pending_removals.device_id`는 의도적으로 `devices(id)`에 FK가 없어(마이그레이션 0020 참조) 삭제된 device row와 독립적으로 존재하므로, 이 행만으로는 device ↔ user 매핑을 재구성할 수 없음(그 매핑은 이미 사라진 `devices` 행에 있었음). cross-group 상관관계 자체는 device 생존 중에 이미 `group_members`로 가능했으므로 새로운 상관관계 능력이 아니라 **그 상관관계가 살아 있는 기간이 늘어난 것**. 또한 이 행이 지워지는 조건(`created_at_epoch < groups.epoch`)은 "이 device의 Remove가 실제로 착지했다"는 암호학적 증명이 아니라 "이 알림 이후 아무 Commit이나 하나 착지했다"는 사실만 증명하는 휴리스틱임(서버는 Commit 내용을 볼 수 없음, RFC 9420 §6/§12.4) — 자세한 내용은 `GroupRepository::delete_pending_removal` 문서 참조.
