...
**cycle 493 추가 — T3: 전달 순서를 통한 영구 메시지 파괴 (issue #2, MLS Commit 수신 배선):** `useMessages.ts`가 처음으로 peer의 MLS Commit을 실제로 merge하기 시작하면서, DS(Delivery Service)의 envelope 전달 순서(§5.4 항목 2, §4A.5)가 단순 지연/보류를 넘어 **영구적이고 탐지 불가능한 콘텐츠 파괴 레버**가 됨: 그룹은 `max_past_epochs(0)`(`mls_group.rs`)로 설정되어 있어 Commit이 merge되는 즉시 이전 epoch의 메시지 키가 완전히 폐기되므로, DS가 같은 epoch에 속한 Application 메시지보다 그 epoch를 종료시키는 Commit을 먼저 배달하면 해당 Application 메시지는 되돌릴 수 없이 소실됨. 클라이언트가 이 순서를 검증할 독립적 수단은 없음(§5.4 항목 2가 명시하듯 순서 보장은 DS의 책임으로 위임됨). 구현된 완화(`app/src/hooks/useMessages.ts`의 단일 poll 커서 + Commit/Application 공유 head-of-line 지연 큐, 발신자별 상한 포함)는 **클라이언트 측 우발적 재정렬**만 방지하며, DS가 의도적으로 선택한 배달 순서 자체는 막지 못함 — 이는 기밀성(confidentiality) 불변식을 침해하지 않지만(서버는 여전히 plaintext를 보지 못함), 가용성/무결성(availability/integrity) 측면에서 DS에게 새로 생긴 능력이므로 T3 대응 열에 "완전한 방어" 대신 "부분 대응, 추적 항목"으로 정직하게 기록함. 상세 설계 근거는 `docs/decisions/0005-mls-commit-receive-without-policy-gate.md` 참조.

**cycle 493 추가 — T4 파생: 멤버십 무결성 (issue #2, MLS Commit 수신 배선):** 위 T3와 혼동하지 않도록 구분: DS는 유효한 MLS Commit을 **구성할 수 없음**(Commit 수락에는 현재 그룹 멤버의 서명이 필요하고, 서버는 MLS LeafNode 암호화 자료를 알지 못함 — §3.3 "서버가 알지 못하는 것" 참조). 그러나 **침해되었거나 악의적인 현재 그룹 멤버 디바이스**(T4의 파생 케이스)는 이제 protocol-legal한 Commit 하나로, 애플리케이션 레벨의 정책 검증(veto) 없이, 다른 임의의 멤버를 축출하거나 공격자 디바이스를 그룹에 추가할 수 있음 — `mls_process_commit`(`wasm_exports.rs`)이 의도적으로 채택한 accepted-risk 설계이며(admin-only-Remove 같은 정책이 아직 존재하지 않으므로 2단계 inspect/confirm API 대신 원샷 merge를 사용), 자기 자신이 축출된 경우에만 UI 배너로 피해자에게 신호가 가고(`GroupRemovedBanner`, `ChatLayout.tsx`), 제3자를 축출하거나 새 멤버를 추가하는 경우는 bystander에게 그룹/DM Safety Number 재계산이라는 간접적 신호만 감(§5.6, 사용자가 능동적으로 대조해야 함). MLS 자체의 멤버십 인증에 의해 경계 지어지는 능력이며(임의의 외부자는 불가능), PCS(Post-Compromise Security) 자체가 이 배선 이전에는 도달 불가능했으므로 이는 순수한 퇴보가 아니라 "PCS를 실제로 달성 가능하게 만드는 대가로 감수하는 잔여 위험"임. 상세는 `docs/decisions/0005-mls-commit-receive-without-policy-gate.md` 참조.
...
- **`(media_id, device_id)` — 미디어 다운로드 ACK 존재 여부** (§9.4.3 미디어 GC): 어떤 device_id가 어떤 media_id를 실제로 수신·검증했는지가 `media_acks` 테이블에 영구 저장됨. GC 알고리즘이 필요로 하는 것은 집합 소속 여부뿐이므로(P5 미니멀리즘) 타임스탬프 컬럼은 두지 않음. 다운로드 URL 발급 자체는 기존에도 매 요청마다 서버가 알 수밖에 없는 이벤트였으나(요청 로그), 그 사실을 **영구 저장**한다는 점이 이번에 새로 추가된 메타데이터 카테고리임. (cycle 289 ack-timing 수정, cycle 290 문서 정정) ACK는 더 이상 다운로드 URL 발급 시점이 아니라, 클라이언트가 blobHash 검증과 AES-GCM 복호화에 모두 성공한 뒤 별도의 `POST /v1/media/:id/confirm-download` 호출로 기록됨 — 즉 "ACK = 다운로드 URL 발급"이 아니라 "ACK = 수신자가 실제로 온전한 plaintext를 복호화 검증함"임(단, "열람"까지는 아님 — UI에서 실제로 보거나 재생했는지는 여전히 서버가 알 수 없음). §3.4의 GC 타이밍 오라클이 이전보다 더 정밀한 신호를 노출하게 되었으므로 함께 참조.
- **`(envelope_id, device_id)` — 브로드캐스트 envelope ACK 존재 여부** (cycle 350, 메시지 삭제 접근 제어 수정): 그룹 브로드캐스트 envelope(수신자 없음 — Application 메시지 및 MLS Commit)를 어떤 device_id가 실제로 폴링·ACK했는지가 `envelope_acks` 테이블에 영구 저장됨(`media_acks`와 동일하게 타임스탬프 없이 집합 소속 여부만, P5 미니멀리즘). 수정 전에는 그룹의 **아무** 디바이스나 첫 ACK만으로 envelope을 즉시 삭제할 수 있어(멤버십 검증 없음) 메시지 검열이나 Commit 삭제를 통한 그룹 영구 desync가 가능했음(보안 감사에서 발견한 HIGH 등급 결함). 수정: ACK는 현재 그룹 멤버만 가능하며(`check_sender_is_member`), envelope은 발신자를 제외한 **모든** 현재 멤버가 ACK해야만 삭제됨(발신자는 자신의 브로드캐스트를 MLS 복호화 실패로 인해 클라이언트에서 애초에 ACK하지 않으므로 제외 — `media_service.rs`의 업로더 제외 패턴과 동일). `(envelope_id, device_id)` 쌍은 `media_acks`보다 더 세밀한 신호임 — 미디어는 그룹당 드물게 공유되지만 envelope ACK는 **모든 메시지**에 대해 기록되어, `envelope_id`를 `envelopes.sender_device_id` 및 `created_at`과 결합하면 어떤 디바이스가 어떤 메시지를 언제(폴링 주기 단위로) 실제 수신·복호화했는지 유추 가능. ACK 행은 envelope 삭제 시 `ON DELETE CASCADE`로 함께 삭제되므로 정상 GC 경로에서는 일시적이나, 멤버가 폴링 없이 그룹을 이탈하는 등 전원 ACK 조건이 영원히 충족되지 않는 경우를 대비해 explicit TTL이 없는 envelope에도 기본 30일 보존 상한(§11.4)이 함께 추가됨(`PgEnvelopeRepository::delete_expired`) — 아래 §3.4에 GC 타이밍 오라클 확장 내용 참조. **cycle 493 추가 (issue #2, MLS Commit 수신 배선):** Commit 타입 envelope에 대한 ACK의 의미가 "폴링해서 봤음"에서 "이 디바이스가 이 Commit을 실제로 로컬 MLS 상태에 merge하는 데 성공했음"으로 강화됨(`useMessages.ts`) — merge에 실패한 Commit은 절대 ACK하지 않는 것이 정확성 요건이기 때문(§5.4의 "이미 merge하지 않은 것을 ACK하면 DS의 유일한 사본이 지워진다" 원칙). 이전에는 Commit이 무조건 즉시 ACK되었으므로 ACK 자체에 암호학적 의미가 없었으나, 이제 ACK의 부재가 서버에 대해 "다른 그룹을 보고 있음" 또는 "이 디바이스의 MLS 상태가 이 Commit을 merge하지 못함(fork, stale epoch, 또는 유계 지연 큐 초과로 드롭됨)"이라는 per-device 신호를 노출함 — 새로운 필드가 아니라 기존 `envelope_acks` 필드의 신호 해상도가 Commit 타입에 한해 상승한 것. 또한 다른 그룹의 Commit은 해당 채팅이 활성화될 때까지 의도적으로 ACK되지 않으므로(위와 동일한 이유), Commit ACK 타이밍이 Application과 마찬가지로 "현재 어느 그룹을 열람 중인지"를 드러내는 기존 신호 카테고리에 동일하게 편입됨.
...
- **미디어 storage_key의 `region_id` — 오브젝트를 생성한 리전** (§9.4.3 고아 객체 스윕, cycle 426): storage_key 포맷이 `media/{region_id}/{uuid}`이므로, 오브젝트를 생성한 리전이 R2 오브젝트 키 자체(클라이언트에게 노출되는 presigned URL 경로 포함)에 영구적으로 각인됨 — Cloudflare R2 운영자의 접근 로그와 버킷 크리덴셜 보유자(버킷 공유 시 타 리전 서버 포함)에게 보임. 위 `home_region`/`region_id` 항목과 같은 카테고리이므로 새로운 기밀성 델타는 아니지만, T5·T7에 대해 "요청 IP ↔ 생산 리전" 및 "리전별 미디어 인벤토리 선별 열거" 상관관계를 storage 레이어에 추가함. 함께, region_prefix당 `{region_id}/.owner` 오브젝트 하나가 존재하며 내용은 해당 환경이 자신의 Postgres에 무작위 생성해 보관하는 owner UUID뿐으로 사용자·디바이스·키·ciphertext와 연결되지 않는 순수 인프라 메타데이터임. 상세(오너 센티널 메커니즘의 실제 보증 범위, 삭제 안전장치)는 §9.4.3 참조.
- **크로스 리전 abuse 차단 신호 — `abuse:ip:<SHA-256>` / `abuse:user:<uuid>` + reason + origin_region, TTL ≤24h** (§6.4 abuse signal 동기화 primitive, cycle 433): 한 리전이 IP/사용자를 로컬 차단하면 `PropagateAbuseSignal` gRPC RPC로 모든 peer 리전에 전파되어 각 리전의 Redis에 최대 24시간 TTL로 저장됨(`powehi-grpc`의 `MAX_ABUSE_SIGNAL_TTL_SECS`). Subject는 SHA-256(IP) 다이제스트 또는 내부 user UUID이며 원본 IP는 저장·전파·로깅되지 않음. 다만 이 다이제스트는 **HMAC이 아닌 평문 SHA-256**이므로(§8.3의 초대 토큰 해시와 달리 입력이 고엔트로피가 아님 — IPv4 공간은 2^32) 다이제스트를 보유한 자(Redis 덤프, 또는 이 값을 수신하는 모든 peer 리전)가 수 초 내에 원본 IP를 역산할 수 있어 **가명화(pseudonymisation)일 뿐 기밀성 보장이 아님** — `crates/domain/powehi-domain/src/abuse.rs` 모듈 문서에 명시된 의도적 한계이며 HMAC-SHA256(mesh-shared secret, `auth_service`의 handle oracle과 동일 패턴)으로의 전환은 미결 항목으로 남음(threat-model-checker cycle 433 F2, crypto-reviewer cycle 434 재확인). IPv4 전수 열거뿐 아니라 IPv6에서도 (전수 열거는 불가능하더라도) 특정 IP가 차단됐는지에 대한 **targeted confirmation**은 동일하게 trivial하며, 이 다이제스트는 키 회전/epoch 없이 mesh 전역·영구적인 stable pseudonym이라 리전 간 사용자 linkage에도 쓰일 수 있음(crypto-reviewer cycle 434). cycle 433 시점에는 이 primitive를 실제로 호출하는 production 코드 경로가 없음(REST 라우트/rate limiter가 아직 연결되지 않음) — §6.4 하단 구현 현황 참조. **HMAC 전환은 F2/F3와 동급으로, `from_ip` 호출부(로컬 rate limiter 트립 지점)를 실제 production 코드에 배선하기 전에 반드시 해결해야 하는 항목으로 격상함(crypto-reviewer cycle 434)** — 미배선 상태에서는 노출되는 다이제스트가 0개이므로 accepted-risk이지만, 배선과 동시에 이 한계가 실제 노출로 바뀜.

- **`(group_id, device_id, created_at_epoch, created_at)` — 대기 중인 MLS Remove 알림** (§5.4 revocation 알림 채널, cycle 448): 디바이스가 revoke되면 서버는 그 디바이스가 속해 있던 각 그룹에 대해 "남은 멤버들이 아직 이 device_id에 대한 MLS Remove를 issue해야 한다"는 알림 행을 `pending_removals` 테이블에 영구 저장함. 값 자체(어떤 device가 어떤 그룹에 속해 있었는지)는 위 `group_members` 항목으로 서버가 이미 알던 사실과 같은 카테고리이지만, `media_acks`/`envelope_acks`와 마찬가지로 **이미 알던 사실을 새로 영구 저장**한다는 점이 델타임: 변경 전에는 `devices` 행 삭제 시 `group_members` cascade로 이 소속 기록이 DB에서 완전히 사라졌으나, 이제는 revoke 이후에도 "device D가 시각 T(`created_at`)에 그룹 G에 속한 채로 revoke되었다"는 사실이 (해당 그룹의 남은 멤버가 실제로 `remove_member`를 호출하고 그 시점 이후 그룹에 어떤 Commit이든 하나 착지할 때까지) 잔존함 — **활성 그룹**(revoke 이후에도 계속 Commit이 오가는 그룹)에 한해서는 무기한이 아니며 사실상의 보존 하한(floor)이 있음(cycle 451): 리더 락(`GC_LOCK_PENDING_REMOVALS`)으로 보호되는 24시간 주기 백그라운드 sweep이 `created_at`이 `pending_removal_sweep_grace_days`(기본 30일, **최소 30일이지 상한이 아님** — 실제 삭제 시점은 30일 + 최대 1 tick + 백로그)보다 오래되고 **동시에** `delete_pending_removal`과 동일한 epoch 게이트(`groups.epoch > created_at_epoch`)를 통과한 행만 삭제함(포트 메서드 `GroupRepository::sweep_stale_pending_removals`, 인덱스는 마이그레이션 0021 `pending_removals_created_at_idx`). **이 sweep은 기본적으로 비활성화(`pending_removal_sweep_enabled` 기본값 `false`, cycle 451)** — 아래 잔여 위험 문단에서 보듯 아직 이 신호를 소비하는 프론트엔드가 없기 때문. epoch 게이트를 나이 조건에 더한 이유는 "알림을 지우고 싶은 당사자에게 비용을 부과"하기 위해서가 **아님** — 이 게이트는 **그룹 활동성(liveness) 필터**일 뿐, 지우려는 특정 멤버에게 비용을 강제하지 않음: 그룹 내 *아무* 멤버의 *아무* Commit(예: RFC 9420 §12.4.3이 권장하는 일상적 self-Update)이든 하나만 착지하면 게이트가 열리므로, 활성 그룹에서 알림이 사라지는 데 드는 한계 비용은 사실상 0(그냥 30일을 기다리면 됨)임 — 이는 `delete_pending_removal`이 지우는 당사자에게 실제로 부과하는 비용(Commit 수락 + 인증된 `remove_member` 호출)과는 다른 성질임. 이 게이트가 실제로 보장하는 것은 훨씬 좁고 여전히 유의미함: **그룹이 revoke 이후 단 하나의 Commit도 받지 못했다면(=남은 멤버 전원이 오프라인이거나 비활성인 경우) 해당 행은 나이와 무관하게, 무기한, 절대 sweep되지 않음** — 정확히 이 테이블이 존재하는 "디바이스 압수, 남은 멤버 전원 장기 부재" 시나리오를 보호함(휴면 그룹에는 사실상 보존 상한이 없음). 한 tick의 삭제량은 `pending_removal_sweep_max_deletes_per_run`(기본 10,000행)으로 상한이 걸려 있어 truncate된 실행은 다음 tick에서 이어짐. `pending_removal_sweep_enabled=false`로 끌 수 있고(Helm `values.yaml`의 `config.pendingRemovalSweepEnabled` 경유, 현재 기본값이 바로 이 `false`), 이 sweep은 보안 통제가 아니라 순수 DB 위생 작업임 — 이 테이블은 알림 채널이지 강제 수단이 아니므로(아래 참조), 조건을 만족한 stale 알림 한 건을 잃는 것은 어떤 강제되는 불변식도 약화시키지 않고 단지 "비정상적으로 오래 미해결인 Remove"에 대한 리마인드를 멈출 뿐임. **잔여 위험(cycle 451 threat-model-checker, accepted risk — sweep이 활성화되기 전에 반드시 재검토):** `GET /v1/groups/:group_id/pending-removals`(§5.4)는 이미 배선되어 있고, 이를 소비하는 프론트엔드가 cycle 453에 배선됨(2단계 사용자 확인, 로컬 대조 없음 — `app/src/components/PendingRemovalBanner.tsx`). 다만 아래에서 보듯 이 정책은 로컬 대조 채널이 없어 sweep 활성화의 전제 조건을 아직 충족하지 못함 — 즉 sweep이 활성화되면, 그룹이 revoke 이후 (문제의 device를 명시적으로 remove하지 않은 채) 다른 이유로 계속 Commit을 주고받는 "활동은 있지만 이 특정 알림에는 아무도 응하지 않는" 경우는 예외적 edge case가 아니라 **모든 활성 그룹에서 기본적으로 벌어지는 상황**이 됨(30일 후 아무도 읽지 않은 채 알림이 지워짐). 이 경우 클라이언트가 대조할 수 있는 대안 채널로 문서들이 인용해 온 "자신의 ratchet tree를 그룹 device 목록과 대조"는, 서버 측 채널인 `GET /v1/groups/:group_id/members`가 cycle 454에 배선되어 **절반만 충족되었음**: 그룹의 현재 멤버는 서버가 기록 중인 device_id 목록을 읽을 수 있으나(멤버 전용, fail-closed, 512개 상한 + `truncated` 플래그, UUID 정렬), **클라이언트가 이를 자신의 ratchet tree와 실제로 대조할 수단은 아직 없음** — WASM의 `mls_group_members`는 `{leafIndex, sigKeyHex}`만 반환하고 device_id ↔ leaf 매핑을 노출하지 않으므로 두 목록을 join할 키가 존재하지 않음. **cycle 456 재검토(crypto-reviewer, NEEDS-REWORK):** `mls_group_members`에 `credentialIdentityHex`(BasicCredential identity bytes) 필드를 추가해 이 join을 시도했으나, 이 코드베이스의 현재 identity 모델에서 그 identity bytes는 `mlsInitIdentityFromPhrase`가 파생시키는 `SHA-256(recovery phrase)[0..16]` — **계정(phrase) 단위 레이블**이며, 같은 phrase로 복구한 모든 device가 동일한 값을 가짐 — 이고, 서버가 발급하는 per-device `device_id`(`crypto.randomUUID()`, `auth_service.rs`)와는 독립적으로 생성되어 **인증된 바인딩이 전혀 없음**(RFC 9420 §5.3은 credential identity의 authenticity를 외부 Authentication Service에 위임하며, 이 코드베이스엔 그런 AS가 없음). 두 값을 그대로 비교하면 정당한 제거 요청도 거의 항상 "not-found"로 표시되어 위조 탐지가 아니라 경고 피로(alarm fatigue)를 유발함 — 따라서 이 필드는 이름을 `credentialIdentityHex`로 정정한 채 WASM에만 남기고 프론트엔드(`PendingRemovalBanner.tsx`)에는 배선하지 않았음. 따라서 sweep 활성화의 전제는 **여전히, 그리고 이전에 생각했던 것보다 더 근본적으로 미충족**이며, 남은 후속 항목은 (a) device_id를 MLS credential identity에 실제로 바인딩하도록 등록/복구 흐름을 바꾸는 것(별도 plan + threat-model-checker 대상), 또는 (b) 이미 배선된 §5.6 safety number를 T3에 대한 실질적 로컬 신뢰 근거로 재평가하는 것임. 또한 이 엔드포인트는 서버가 저장하는 `(group_id, device_id)` 매핑(위 `group_members` 항목)에 **새로운 카테고리를 추가하지 않으며**(서버가 새로 알게 되는 것은 없음), 델타는 "이미 서버가 알던 사실이 이제 그룹 멤버에게 읽기 가능해졌다"는 노출 방향뿐임 — 단 T4(디바이스 압수/세션 토큰 탈취) 관점에서는 새 표면임: 로컬 클라이언트는 `sigKeyHex`만 보관하므로, 탈취된 세션 토큰 보유자가 동료 멤버의 device UUID 목록을 획득하는 경로는 이 엔드포인트로 처음 생김(가명 UUID이며 user 매핑은 포함하지 않음). 완화: `pending_removals.device_id`는 의도적으로 `devices(id)`에 FK가 없어(마이그레이션 0020 참조) 삭제된 device row와 독립적으로 존재하므로, 이 행만으로는 device ↔ user 매핑을 재구성할 수 없음(그 매핑은 이미 사라진 `devices` 행에 있었음). cross-group 상관관계 자체는 device 생존 중에 이미 `group_members`로 가능했으므로 새로운 상관관계 능력이 아니라 **그 상관관계가 살아 있는 기간이 늘어난 것**(다만 활성 그룹에 한해 활성화 시 최소 30일 — 휴면 그룹은 여전히 무기한). 또한 이 행이 지워지는 조건(`created_at_epoch < groups.epoch`)은 "이 device의 Remove가 실제로 착지했다"는 암호학적 증명이 아니라 "이 알림 이후 아무 Commit이나 하나 착지했다"는 사실만 증명하는 휴리스틱임(서버는 Commit 내용을 볼 수 없음, RFC 9420 §6/§12.4) — 자세한 내용은 `GroupRepository::delete_pending_removal`/`GroupRepository::sweep_stale_pending_removals` 문서 참조.

- **`groups.epoch` — 그룹의 누적 MLS Commit 수락 횟수** (`GET /v1/groups/:group_id/epoch`, cycle 499, issue #2 admin-initiated Remove UI의 선행 단계): 서버는 `GroupRepository::advance_epoch`의 CAS를 통해 이미 이 카운터를 저장·관리하며(`AddMemberRequest.epoch`로 이미 클라이언트→서버 방향으로 흘러들어오는 값과 같은 카테고리), 이 엔드포인트는 그 값을 그룹 멤버가 읽어올 수 있게 할 뿐 — 서버가 새로 알게 되는 사실은 없다(threat-model-checker cycle 499, GREEN 조건부). 다만 T4(디바이스 압수/세션 토큰 탈취) 관점에서는 새 표면임: envelope은 전원 ACK 후 삭제되고 기본 30일 보존 상한(§11.4)이 있어 폴링만으로는 재구성할 수 없는 "그룹 생성 이래 누적 Commit 수(=key rotation/멤버십 변경 총량, GC로 소실된 과거 구간 포함)"라는 조악한(coarse) 신호를, 탈취된 세션 토큰 보유자가 이 엔드포인트로 처음 직접 획득할 수 있게 됨. 서버가 보고하는 값은 §4A.5의 CAS precondition 토큰(불투명 정수)일 뿐이며, 클라이언트의 로컬 MLS 상태 판단(재동기화 트리거, ratchet tree 판정)의 입력으로 사용해서는 안 됨 — T3는 임의의 값을 반환할 수 있으므로, 그렇게 쓰면 §5.4 항목 5가 `pending_removals`에 대해 경고한 것과 같은 결함이 생김. 아래 §3.5.1의 리전-로컬 한계 중 integrity 측면(stale-or-zero 값이 권위 있는 것처럼 반환되는 것)은 같은 사이클에 `RegionMismatch` fail-closed 가드로 해소되었으나(security-auditor cycle 499 PASS), 가용성 한계(home region 밖 멤버는 이 엔드포인트를 아예 쓸 수 없음)는 남아 있으므로, 실제 `sendCommit` 재시도 배선(step b) 설계 시 이를 함께 고려할 것.
...
**침해된/악의적 peer 리전의 blast radius 확장 (cycle 433, threat-model-checker F3 — 미결 accepted-risk 항목):** §6.4 abuse signal 전파(`PropagateAbuseSignal` gRPC RPC)가 도입되면서, 위 문단의 "해당 리전의 메타데이터만 노출됨"이라는 전제가 abuse 차단 신호에 대해서는 더 이상 성립하지 않습니다 — 침해된 리전 X는 **자신과 한 번도 접촉한 적 없는** 사용자/IP에 대한 abuse 차단 신호를 수신하게 됩니다(mesh 전체 전파이므로). 더 심각하게는, 현재 구현(`crates/adapters/inbound/powehi-grpc/src/server.rs`의 `propagate_abuse_signal`)이 mTLS peer 인증서로 **"어느 리전이 이 신호를 보냈는가"만 검증**하고 **"그 리전이 이 특정 subject(user_id)에 대해 차단을 선언할 권한이 있는가"는 검증하지 않으므로**, 리전 하나가 침해되면 그 리전과 무관한 임의 user_id에 대해 mesh 전역(모든 peer 리전) 최대 24시간 차단을 유발할 수 있습니다 — 타겟 사용자에 대한 크로스 리전 서비스 거부 벡터입니다. cycle 433 시점에는 이 RPC의 수신 결과(`AbuseSignalStore::is_blocked`)를 실제로 조회하는 production 코드 경로가 없어(§3.3 참조) 현재는 잠재적 위협이며 실효는 없습니다. **이 RPC를 실제로 강제(rate-limit 게이트 요청 등)에 연결하기 전에 반드시 해결해야 함**: (i) `AbuseSubject::User`는 로컬에 알려진 `home_region == origin_region`일 때만 수용하거나, (ii) 외부 리전에서 온 신호는 로컬 판정보다 훨씬 짧은 advisory TTL만 부여하거나, (iii) 2개 이상 리전의 corroboration을 요구. 미해결 상태로 두는 것은 이 항목이 아직 강제되지 않는 동안에만 accepted-risk입니다.

**`AbuseSubject::IpHash`는 위 (i) 완화책이 적용되지 않으며 더 심각함(threat-model-checker cycle 434 재검토):** `IpHash`에는 애초에 "home region" 개념이 없으므로 (i)의 `home_region == origin_region` 검증이 subject 종류로는 아예 적용될 수 없습니다 — 침해된 리전 하나가 CGNAT/캐리어 egress IP 하나의 다이제스트를 차단 선언하면, 그 IP를 공유하는(자신과 전혀 무관한 지역의) 수천 명의 무고한 사용자가 mesh 전역에서 최대 24시간 동안 함께 차단될 수 있습니다 — User subject보다 파급 범위가 넓은 변종입니다. `IpHash`에 대해서는 (ii) advisory TTL 단축 또는 (iii) corroboration 요구만이 적용 가능한 완화책이며, 배선 전 반드시 이 subject 종류를 명시적으로 다뤄야 합니다.

**`pending_removals`/`RemovalRequired`는 현재 리전-로컬이며 크로스 리전 복제 대상이 아님(cycle 448, threat-model-checker):** §5.4의 revoke 알림은 revoke를 처리한 리전의 Postgres에만 기록되고(`SyncGroupMembership` gRPC 복제 대상에 포함되지 않음), `RemovalRequired` 이벤트는 해당 리전 pod-local WS hub로만 dispatch됩니다(리전 간 이벤트 버스 포워딩 미구현). 즉 그룹의 home region이 revoke를 처리한 리전과 다르거나, 남은 멤버가 다른 리전에 접속해 있으면 그 멤버는 이 알림을 전혀 받지 못합니다 — PCS 회복을 위한 Remove 유도가 멀티 리전 환경에서는 부분적으로만 동작합니다. 크로스 리전 복제/포워딩은 후속 사이클 항목으로 남습니다. **이제 회복 지평선(recovery horizon)도 존재합니다(cycle 451):** `pending_removals`에 §3.3의 retention sweep이 적용되므로, 크로스 리전 복제/포워딩이 언젠가 구현되더라도 이미 sweep 조건(나이 + epoch 게이트)을 만족해 삭제된 행은 복구할 수 없습니다 — 즉 "복제 미구현"이라는 현재 상태와 "복제되더라도 유예 기간을 넘긴 알림은 어차피 사라져 있다"는 상태가 함께 고려되어야 합니다. **멀티 리전 조합이 sweep의 epoch 게이트 전제를 깨뜨림(cycle 451 threat-model-checker):** sweep의 epoch 게이트는 암묵적으로 "epoch가 올랐다 = 알림을 받은 멤버 중 누군가 그에 반응해 행동했다(혹은 최소한 활동했다)"를 전제하지만, `groups.epoch`는 멤버가 어느 리전에 접속해 있든 그룹의 home region에서 오르고, 알림 자체는 리전-로컬이라 다른 리전에 접속한 멤버는 애초에 알림을 받을 수 없습니다. 즉 알림을 구조적으로 받을 수 없는 멤버가 보낸 일상적 Commit이 바로 그 알림을 30일 후 sweep 가능하게 만드는 조합이 성립하며, 이는 위 §3.3의 "활성 그룹의 알림은 아무도 안 읽어도 사라짐" 잔여 위험을 멀티 리전 환경에서 한층 더 흔한 경로로 만듭니다. **`GET /v1/groups/:group_id/members`도 동일한 리전-로컬 한계를 가짐(cycle 454):** REST 계층에는 그룹 home region으로의 핀/프록시가 없고(`powehi-rest-api`에 `home_region` 참조 없음), peer 리전의 `group_members`는 `SyncGroupMembership` → `upsert_members`가 **add-only**(`ON CONFLICT DO NOTHING`, 스냅샷에 없는 멤버를 삭제하지 않음, `joined_at_epoch`는 0으로 고정)이므로 삭제가 전파되지 않음. 따라서 비-home 리전이 응답하는 멤버 목록은 **단조 증가하는 stale superset**일 수 있으며(이미 제거된 device가 영구히 남음), 응답에는 리전/권위성 표시가 없음. 클라이언트는 이 목록을 권위 있는 멤버십으로 취급해서는 안 되며, 특히 "목록에 있음"을 device가 여전히 그룹에 있다는 증거로 사용해서는 안 됩니다. 크로스 리전 삭제 전파는 후속 사이클 항목.

**`GET /v1/groups/:group_id/epoch`는 `/members`와 같은 계열의 한계에서 출발했으나 integrity 측면은 같은 사이클에 fail-closed로 해소됨(cycle 499, threat-model-checker + security-auditor):** `SyncGroupMembership`이 생성하는 group stub은 `epoch: Epoch(0)`으로 고정되고(`powehi-grpc/src/server.rs`), `upsert_members`는 `ON CONFLICT DO NOTHING`으로 group 행을 갱신하지 않으므로 epoch은 크로스 리전으로 전혀 복제되지 않습니다 — 이 자체는 여전히 사실입니다. 그러나 `GroupService::get_epoch`(`group_service.rs`)는 이제 `group.home_region != self.local_region`일 때 그 stale-or-zero 값을 반환하는 대신 `DomainError::RegionMismatch`(→ `502 region_mismatch`)로 fail-closed하므로(security-auditor F1 재검증 PASS), "실제보다 낮은 epoch이 권위성 표시 없이 반환된다"는 integrity 리스크는 더 이상 존재하지 않습니다. 남은 것은 가용성 한계뿐입니다: 그룹의 home region이 아닌 리전에 접속한 멤버는 이 엔드포인트를 아예 사용할 수 없습니다(§4A.5 항목 4의 재시도 계약에는 애초에 참여 불가). 크로스 리전 epoch 복제 또는 home-region 프록시(non-home 리전에서 온 요청을 home region으로 전달)는 이 가용성 한계를 해소할 후속 사이클 항목으로 남습니다 — issue #2의 admin-initiated Remove UI가 이 엔드포인트를 실제 재시도 경로에 연결할 때(step b) home-region 밖 멤버를 어떻게 다룰지 함께 설계해야 합니다.
...
5. **Revoke된 디바이스에 대한 Remove *요청* 신호 전달 — 구성이 아님** (cycle 448): 디바이스가 revoke되면 서버는 그 디바이스가 속했던 각 그룹의 남은 멤버들에게 "이 device_id에 대한 MLS Remove를 issue해야 한다"는 신호를 durable 알림(`pending_removals`, §3.3)과 실시간 이벤트(`RemovalRequired`)로 전달합니다. 서버는 그룹 상태도 키 자료도 갖지 않으므로 Remove를 **스스로 구성할 수 없으며**, 이 신호는 순수히 클라이언트가 해야 할 일을 알려주는 디스커버리 채널입니다. **중요한 신뢰 경계**: 이 신호는 서버가 발신하므로, 악의적이거나 침해된 서버 운영자는 임의의 `(group_id, device_id)`에 대해 이 신호를 위조하여 정상 디바이스를 그룹에서 축출하도록 정직한 클라이언트를 유도할 수 있습니다(가용성/멤버십 무결성 공격이지 기밀성 침해는 아님 — §3.5.1 T3 참조). 따라서 클라이언트는 이 신호를 **자동으로, 무조건 신뢰하여 실행해서는 안 되며**, 최소한 사용자 확인을 거치거나 자신의 로컬 상태(예: 해당 device_id가 실제로 자신의 디바이스 목록/최근 활동과 불일치하는지)와 대조하는 정책을 두어야 합니다. **확정된 클라이언트 정책(cycle 453, `app/src/components/PendingRemovalBanner.tsx`):** 그룹별 배너로 대기 목록을 표시하되 **디바이스마다 2단계 명시적 사용자 확인**(첫 클릭은 arm만 하고, 두 번째 "확인" 컨트롤은 실수로 같은 좌표를 다시 클릭하는 것을 막기 위해 arm 후 일정 시간 동안 비활성화됨)을 요구하며, 자동 실행 경로는 존재하지 않습니다(fetch 실패 시 fail-closed — 아무것도 표시하지 않음). 배너는 이 신호가 서버 보고일 뿐 검증되지 않았다는 경고 문구를 명시적으로 표시합니다. 단, 이 정책은 위에서 요구한 "로컬 상태와의 대조"를 **아직 포함하지 않습니다**: 서버 측 채널인 `GET /v1/groups/:group_id/members`는 cycle 454에 배선되었으나(멤버 전용, 비멤버·미존재 그룹 모두 401로 응답 수준에서 구분 불가, 512개 상한 + `truncated`), 클라이언트가 이 device_id 목록을 자신의 MLS ratchet tree와 join할 키(device_id ↔ leaf 매핑)를 WASM이 아직 노출하지 않으므로 대조는 여전히 실행 불가능하며, 위조 신호에 대한 유일한 방어선은 계속 사용자 확인 클릭 하나입니다. **cycle 456 재검토:** WASM에 `credentialIdentityHex`를 노출하도록 시도했으나 crypto-reviewer가 NEEDS-REWORK로 반려함 — 이 코드베이스의 MLS credential identity는 계정(phrase) 단위 레이블이며 device_id와 인증된 바인딩이 없어, 그대로 대조하면 정당한 제거도 거의 항상 위조처럼 보이는 alarm fatigue를 유발함(§3.3 참조). 배너에는 배선하지 않았으며, 실질적 로컬 대조는 여전히 존재하지 않음 — 위 §3.3의 후속 항목 (a)/(b) 참조. 또한 이 엔드포인트 자체가 서버가 제공하는 신호이므로, 두 서버 신호(`pending-removals`와 `members`)의 상호 일치는 T3에 대해 아무것도 증명하지 않습니다 — 대조의 유일한 신뢰 근거는 클라이언트의 **로컬** ratchet tree입니다. 또한 이 확인 동작은 서버 측 `group_members` 삭제(향후 envelope fan-out 중단)일 뿐 **MLS Remove commit이 아니므로 그룹 epoch가 진행되지 않고 PCS는 회복되지 않습니다** — 배너 UI는 이를 "제거 완료"로 표현하지 않고 이 한계를 인라인으로 명시하며, MLS Remove가 실제로 클라이언트에서 착지하기 전까지는 부분 완화로만 취급합니다. **이 신호는 영구적이지 않을 수 있습니다(cycle 451):** durable 알림(`pending_removals`)에는 §3.3의 retention sweep이 적용될 수 있어, 활성화되면 활성 그룹에서는 30일 유예 기간(및 epoch 게이트) 경과 후 사라질 수 있습니다(휴면 그룹은 무기한 보존). 이 sweep은 정확히 이 이유로 **기본 비활성화**되어 있으며, §3.3에 기록된 대로 이 신호를 소비하는 실제 프론트엔드 정책이 확정되기 전까지는 활성화해서는 안 됩니다 — 활성화 이후에는 프론트엔드 정책이 "서버가 언젠간 알려주겠지"를 전제해서는 안 됩니다. **프론트엔드가 cycle 453에 배선됨**(2단계 사용자 확인, 로컬 대조 없음 — 위 §3.3 잔여 위험 문단 및 `app/src/components/PendingRemovalBanner.tsx` 참조). 그러나 sweep 활성화 전제(그룹 device-list 대조 능력 — 엔드포인트 + device_id↔leaf 매핑)는 **여전히 미충족**이므로 `pending_removal_sweep_enabled`는 계속 `false`로 유지합니다.
...
**구현 현황 (cycle 433):** 전파 primitive는 구현되어 있으나(`crates/domain/powehi-domain/src/abuse.rs`, `AbuseSignalStore` 포트, `PropagateAbuseSignal` gRPC RPC — `origin_region` mTLS 바인딩, TTL 24h clamp, per-origin-region rate limit으로 하드닝됨) **아직 어떤 production 코드 경로도 이를 호출하지 않음**: 로컬 rate limiter(`powehi-rest-api`의 `HandleRateLimiter`/`tower-governor`)가 차단 시 `broadcast_abuse_signal`을 트리거하지 않고, 어떤 라우트 핸들러도 `AbuseSignalStore::is_blocked_or_allow`를 조회하지 않음. `AppState` 배선(`powehi-rest-api/src/lib.rs`, 15곳 이상의 생성 지점)은 후속 사이클 작업으로 남김. 배선 전 반드시 §3.5.1의 F3(subject 인가 부재로 인한 크로스 리전 targeted DoS 벡터 — `IpHash`는 더 심각한 변종, 위 재검토 참조)와 §3.3의 HMAC 전환 항목 해결 필요 — 그 전까지 이 primitive는 리전 간 mTLS 인증서 RPC 표면만 넓히고 실제 방어 이득은 0.

**cycle 434 하드닝:** security-auditor가 `abuse_signal_limiter`(governor 키드 rate limiter)의 내부 맵이 `retain_recent()` 없이는 절대 회수되지 않아, `tls_required=false` fail-open 경로(공격자가 `origin_region` 문자열을 임의로 선택 가능)에서 무제한 메모리 성장으로 이어질 수 있음을 지적 — `bin/powehi-server`의 기존 `HandleRateLimiter` GC 패턴과 동일하게 1시간 주기 GC 태스크를 추가함(`RegionGrpcServer::abuse_signal_limiter_handle`). `governor` 크레이트는 `powehi-rest-api`와 `powehi-grpc` 두 곳에 개별 pin되어 있던 것을 workspace dependency로 승격(버전 drift 방지). rate limiter 자체는 *호출 빈도*만 제한하며 맵의 *키 공간*은 별도로 GC가 담당함 — 이 둘을 혼동하지 않도록 코드 주석도 함께 수정.

**Fail-open 정책:** `AbuseSignalStore::is_blocked`는 저장소 오류 시 "차단 아님"으로 fail-open함(`is_blocked_or_allow` 기본 메서드로 타입에 인코딩됨, threat-model-checker cycle 433 F4) — 위 다이어그램상 1단계 `tower-governor`(인메모리)가 Redis 장애 중에도 계속 동작하므로, abuse 판정을 fail-closed로 두면 캐시 장애 하나가 §6.4가 막으려는 바로 그 결과(전 mesh DoS)를 낳음. 트레이드오프: Redis 장애 중에는 `AuthBruteForce` 등 크로스 리전 차단이 조용히 무효화됨 — 알람 미구현(follow-up).
...
- 모든 수신자가 ACK 한 blob은 N일 후 자동 삭제. **ACK의 정의** (cycle 289 ack-timing 수정, cycle 290 문서 정정): 수신자 디바이스가 `POST /v1/media/:id/confirm-download`를 호출한 시점 — 이는 클라이언트가 blobHash 검증과 AES-GCM 복호화에 모두 성공한 뒤에만 발생함(`get_download_url`은 더 이상 ACK를 기록하지 않음 — URL 발급은 전송 성공을 의미하지 않으므로). 단, 클라이언트가 실제로 콘텐츠를 사용자에게 "열람"시켰는지까지는 여전히 확인 불가(서버는 클라이언트 로컬 UI 상태를 알 수 없으므로 원천적으로 확인 불가) — 또한 `confirm-download` 호출 자체는 best-effort(fire-and-forget)이므로 네트워크 실패 시 ACK가 누락될 수 있으며, 이는 GC를 지연시킬 뿐 안전 방향의 실패임. 그룹에 공유되지 않은 blob(업로더 전용)은 대기할 수신자가 없으므로 유예 기간만으로 GC 대상이 됨.
...
- **고아 객체 스윕 (cycles 422/424):** `delete()`/`run_gc`는 S3 객체를 먼저 삭제한 뒤 Postgres 행을 제거합니다. 클라이언트의 presigned PUT은 발급 시점부터 `r2_presign_upload_ttl_secs` 동안 `delete()`의 실행 시점과 무관하게 유효하므로, 어떤 id에 대해 `delete()`가 이미 완료된 후 그 PUT이 뒤늦게 도착하면 R2에 객체가 새로 생기지만 대응하는 `media_blobs` 행은 없고, `delete()`를 다시 호출해도 `find_by_id`가 `None`을 반환해 S3 삭제 자체를 건너뛰므로 영원히 도달 불가능합니다. 6시간마다 버킷을 전수 열거해 DB와 대조하는 별도 백그라운드 잡(`R2MediaAdapter::sweep_orphaned_storage_objects`)이 이 상태를 정리합니다. 읽는 것은 오브젝트 키와 `LastModified` 타임스탬프뿐이며 본문(ciphertext)은 결코 읽지 않고, 로그에는 삭제 개수만 남습니다 — §3.4의 GC 타이밍 오라클에도 영향 없음(이미 `find_by_id`가 `None`인 상태를 다룰 뿐).
  - **새로운 영구 메타데이터:** storage_key 포맷이 `media/{uuid}`에서 `media/{region_id}/{uuid}`로 바뀌어, 객체를 생성한 리전이 오브젝트 키 자체(클라이언트에게 노출되는 presigned URL 경로 포함)에 영구적으로 각인됩니다. 이는 Cloudflare R2 운영자의 접근 로그와 버킷 크리덴셜 보유자(버킷이 공유된 경우 타 리전 서버 포함)에게도 보입니다. 민감도는 낮음(§3.3이 이미 `home_region`을 서버 인지 항목으로 명시) — 다만 T5(글로벌 패시브 관찰자)·T7(리전 관할 공격자)에 대해 "요청 IP ↔ 생산 리전" 및 "리전별 미디어 인벤토리 선별 열거" 상관관계를 storage 레이어에 새로 추가합니다. (cycle 426) 여기에 더해 `media/{region_id}/.owner` 오브젝트 하나가 각 region_prefix당 추가로 존재합니다 — 내용은 해당 환경이 자신의 Postgres(`media_region_owner` 테이블)에 무작위로 생성해 보관하는 UUID 하나뿐이며, 사용자·디바이스·키·ciphertext와는 연결되지 않는 순수 인프라 메타데이터입니다. 버킷 쓰기 권한을 가진 공격자는 이를 위조·삭제할 수 있지만, 그런 공격자는 이미 버킷 전체에 `DeleteObjects`를 실행할 수 있는 T3/T7 범주에 속하고 미디어 자체가 E2EE이므로 새로운 기밀성 델타는 없습니다(threat-model-checker, cycle 426).
  - **region-prefix 스코핑의 실제 보증 범위:** storage_key의 `{region_id}` prefix로 스윕의 LIST를 스코프하는 것은 **서로 다른 region_id를 쓰는 리전들이 버킷을 공유하는 경우**에만 상호 고아-스윕 오염을 구조적으로 막습니다. **같은 region_id와 같은 버킷을 쓰는 두 환경**(예: 이 저장소의 `values-staging.yaml`/`values-prod-eu.yaml`이 둘 다 `region: eu-frankfurt`이고, 실제 R2 값이 설정되기 전까지는 둘 다 컴파일 기본 버킷 `powehi-media`로 수렴)은 이 스코핑만으로 보호되지 않습니다. 대신 `AppConfig::validate()`가 `region_id != "local"`인데 `r2_bucket`이 컴파일 기본값(`"powehi-media"`)이면 기동을 거부합니다(r2_endpoint의 기존 dev-default 가드와 동일 패턴) — 다만 이는 "버킷 설정을 깜빡함"만 잡을 뿐, "두 환경에 실수로 같은 실제 버킷을 설정함"은 잡지 못하므로, 환경별로 서로 다른 실제 버킷을 쓰는 것은 여전히 운영 규율에 의존합니다. `region_id`는 `validate()`에서 비어있지 않고 `[a-z0-9-]+`만 허용하도록 검증됩니다(`/` 등이 섞이면 한 리전의 prefix가 다른 리전 키 공간의 진부분 prefix가 될 수 있어 격리가 깨짐).
    - **오너 센티널(cycle 426, `media_region_owner` + `{region_prefix}.owner`):** 위 잔여 위험을 *좁히되 완전히 닫지는 않습니다*. 각 환경은 자신의 Postgres에 무작위 owner_id를 한 번 생성해 영구 보관하고, 처음 스윕을 실행할 때 `{region_prefix}.owner`를 조건부 쓰기(`If-None-Match: *`)로 선점합니다. 이후 매 스윕(부팅 시 1회가 아니라 매 실행마다 재확인)마다 R2에 저장된 값을 자신의 owner_id와 비교해, 일치하지 않으면 그 실행에서 아무것도 삭제하지 않고 `gc_orphan_owner_mismatch`를 로그로 남깁니다. 이 메커니즘이 실제로 보장하는 것은 정확히: (1) 같은 prefix를 두고 경쟁하는 두 환경 중 **최대 한쪽만** 선점에 성공하므로 — 이전처럼 두 환경이 서로의 데이터를 상호 삭제하는 시나리오는 제거됩니다; (2) 선점에 진 쪽은 조용한 데이터 손실 대신 영구적이고 시끄러운 탐지 신호를 받습니다. 이 메커니즘이 보장하지 **못하는** 것: 선점에 **이긴** 쪽의 스윕은 이후에도 계속 실행되며, 같은 prefix를 공유하는 (아직 살아있는) 상대 환경의 오브젝트를 자신의 `media_blobs`에 행이 없다는 이유로 "고아"로 판정해 계속 삭제할 수 있습니다 — 즉 상호 파괴는 막지만 일방향 파괴는 막지 못합니다. 따라서 환경별로 서로 다른 실제 버킷을 쓰는 것은 여전히 하드 요구사항이며, 이 메커니즘은 격리(isolation)가 아니라 "일방향 보호 + 탐지"로 정확히 서술해야 합니다(threat-model-checker + security-auditor, cycle 426 — 두 리뷰 모두 코드 주석/커밋 메시지가 "구조적으로 닫음"이라고 과장하지 않도록 명시적으로 요구).
  - **삭제 안전장치:** (1) 6시간 tick당 최대 삭제 개수 상한(`media_orphan_sweep_max_deletes_per_run`, 기본 500); (2) 누적 고아 비율이 임계치(50%, 최소 표본 50개 이상)를 넘으면 아무것도 더 삭제하지 않고 즉시 중단하는 circuit breaker — 임계치는 두 환경이 버킷+region_id를 모두 공유해 서로를 약 50/50 비율로 "고아"로 보는 시나리오까지 잡도록 80%에서 하향(cycle 424); (3) 표본이 최소 표본(50) 미만인 동안에는 누적 삭제량을 별도의 작은 상한(5개)으로 제한하는 pre-sample cap — circuit breaker가 아직 평가할 수 없는 구간에서 소규모/신규 버킷이 무제한으로 삭제되는 것을 막음(cycle 424); (4) 명시적 kill switch(`media_orphan_sweep_enabled`, 기본 true — Helm 차트의 `default true` 템플릿 함수가 boolean `false`를 empty로 취급해 조작자가 끄려고 해도 조용히 `true`로 되돌아가던 버그를 cycle 424에서 수정)와 부팅 직후 즉시 실행을 건너뛰는 1-tick 지연.
