...
impl UserRepository for PgUserRepository {
    async fn save(&self, user: &User) -> Result<(), DomainError> {
        sqlx::query(
            "INSERT INTO users (id, handle_hash, opaque_password_file, recovery_pubkey, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6)
             ON CONFLICT (id) DO UPDATE
               SET handle_hash          = EXCLUDED.handle_hash,
                   opaque_password_file = EXCLUDED.opaque_password_file,
                   recovery_pubkey      = EXCLUDED.recovery_pubkey,
                   updated_at           = EXCLUDED.updated_at",
        )
        .bind(user.id.as_uuid())
        .bind(&user.handle_hash)
        .bind(&user.opaque_password_file)
        .bind(user.recovery_pubkey.as_deref())
    ...
    async fn find_by_id(&self, id: &UserId) -> Result<Option<User>, DomainError> {
        let row = sqlx::query_as::<_, UserRow>(
            "SELECT id, handle_hash, opaque_password_file, recovery_pubkey, created_at, updated_at
